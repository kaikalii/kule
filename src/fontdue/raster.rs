...
use crate::fontdue::math::Line;
use crate::fontdue::platform::{abs, as_i32, copysign, f32x4, fract};
use std::vec::*;
...
impl Raster {
    ...
    pub fn draw(&mut self, lines: &[Line], scale: f32, offset_x: f32, offset_y: f32) {
    ...
    fn line(&mut self, line: &Line, coords: f32x4, params: f32x4) {
        ...
        let (mut target_x, mut target_y, _, _) = (coords + line.adjustment)
            .sub_integer(line.nudge)
            .trunc()
            .copied();
    ...
    pub fn get_bitmap(&self) -> Vec<u8> {
        crate::fontdue::platform::get_bitmap(&self.a, self.w * self.h)
    }
}
