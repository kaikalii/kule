use crate::fontdue::layout::GlyphRasterConfig;
use crate::fontdue::math::{Geometry, Line};
use crate::fontdue::platform::{as_i32, ceil, fract, is_negative};
use crate::fontdue::raster::Raster;
use crate::fontdue::FontResult;
use core::mem;
...
impl Font {
    ...
    pub fn from_bytes<Data: Deref<Target = [u8]>>(
        ...
        for mapping in char_to_glyph.values() {
            ...
            geometry.reposition();
            let bounds = geometry.effective_bounds.unwrap_or_default();
    ...
    pub fn lookup_glyph_index(&self, character: char) -> usize {
        unsafe {
            mem::transmute::<Option<NonZeroU16>, u16>(
                self.char_to_glyph.get(&(character as u32)).copied(),
            ) as usize
        }
    }
}
