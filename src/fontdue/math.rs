use crate::fontdue::platform::{abs, atan, clamp, f32x4};
use crate::fontdue::{FontSettings, AABB};
use std::vec::*;
...
impl CubeCurve {
    ...
    fn angle(&self, t: f32) -> f32 {
        abs(atan(self.slope(t))) * 180.0 / std::f32::consts::PI
    }
}
...
impl Point {
    ...
    pub fn angle_to(&self, other: Point) -> f32 {
        atan((other.y - self.y) / (other.x - self.x)) * 180.0 / std::f32::consts::PI
    }
}
